...
Penalties: contact hold 8; for a source-backed public in-person event, unresolved participation prerequisites 8 instead of the contact-hold penalty; no dated decision point 5; each material unknown 2 (cap 10); unreviewed relationship 3. These do not confer permission. Minimum score defaults to 55; caller changes must stay 40–100. A high price does not itself earn business-value points. Unknown pipeline/host demand is a material limitation, not a buyer-traffic promise. A current active listing without source-backed buyer fit, hosting need or market-position business case is excluded even if its other scores are high.

Every retained card shows a class separately from its score: MONEY NOW for a qualifying seller signal or supported hosting case; PIPELINE for a dated professional event/project within 30 days; NETWORK / KNOWLEDGE for service/resources without current demand. These are reviewable implementation rules, not conversion claims or a release approval. A high-scoring service directory entry remains NETWORK / KNOWLEDGE. The board's separate class counts prevent it from inflating the sales-opportunity count.
